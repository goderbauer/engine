...
class EmbedderSurfaceMetal final : public EmbedderSurface,
 ...
 public:
  struct MetalDispatchTable {
    std::function<bool(GPUMTLTextureInfo texture, int64_t view_id)>
        present;  // required
 ...
 private:
  ...
  // |GPUSurfaceMetalDelegate|
  GPUMTLTextureInfo GetMTLTexture(const SkISize& frame_size,
                                  int64_t view_id) const override;

  // |GPUSurfaceMetalDelegate|
  bool PresentTexture(GPUMTLTextureInfo texture,
                      int64_t view_id) const override;
